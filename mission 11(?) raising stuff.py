...
async def turning_for_degree_v2(degree:int, speed:int=200, ref_yaw:int|None=None, turning_direction:int=1, speed_reduce_angle_diff:int=100, speed_reduce_ratio:float=0.1, tolerance=2):
    ...
    while True:
        ...
        angle_diff = abs(target_yaw - abs(cur_yaw_in_3600()))
...
async def main():
    ...
    print("started")
    """
    await move_straight_for_time(1900, 600)
    await turning_for_degree_v2(890)
    await runloop.sleep_ms(100)
    await move_straight_for_time(167, 467, -1)
    """
    await motor.run_for_degrees(port.D, 220, 400)
