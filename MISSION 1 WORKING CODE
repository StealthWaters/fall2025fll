...
motor.run_to_relative_position(port.C, 130, 400)


async def main():
    #🡇 𝗧𝗬𝗣𝗘 𝗜𝗡 𝗛𝗘𝗥𝗘 🡇
    print("started")
    await motor_pair.move_for_degrees(motor_pair.PAIR_1, 795, 0, velocity=400) #Move towards mission 1
    await runloop.sleep_ms(500)
    await motor_pair.move_for_degrees(motor_pair.PAIR_1, -140, 0, velocity=300) #Move backwards to push the shovel
    await runloop.sleep_ms(100)
    await motor.run_for_degrees(port.C, -40, 200) #Raise attachment
    await runloop.sleep_ms(1000)
    await motor_pair.move_for_degrees(motor_pair.PAIR_1, 70, 0, velocity=300) #Move forwards to pick up the shovel
    await motor.run_for_degrees(port.C, 67, 400) #Put attachment down
    await runloop.sleep_ms(200)
    await motor.run_for_degrees(port.C, -100, 400) #Raise attachment
    await sound.beep()

    #await motor_pair.move_tank_for_degrees(motor_pair.PAIR_1, 41, 40, 40)


    #await turning_for_degree(-100)
    #await move_straight_for_time(100)
    print("ended")
runloop.run(main())
