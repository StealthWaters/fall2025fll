...
#of the mission start and the fork all the way back
# Mission 1
    ...
    await motor_pair.move_for_degrees(motor_pair.PAIR_1, 250, 0, velocity=200)
    await motor.run_for_degrees(port.A, 95, 160) # Raise the fork up to pick up the brush
    await motor_pair.move_tank_for_degrees(motor_pair.PAIR_1, -20, -185, 250) # Turn to sweep to the right
    await runloop.sleep_ms(100)
    await motor.run_for_degrees(port.A, -170, 999999999, ) # Raise fork up to pick up the brush
runloop.run(main())
